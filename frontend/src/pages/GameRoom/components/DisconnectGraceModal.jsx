...
import graceStyles from '../style/disconnect-grace.module.css';
...
export function DisconnectGraceModal({ room })
{
	...
	return (
		<div className={`${cardsStyles.card} ${resultStyles.resultCard} ${graceStyles.disconnectGraceCard}`} role="dialog" aria-modal="true" ref={modalRef} tabIndex="-1">
			<p className={`${introCardsStyles.sectionKicker}`}>
				{t('grace.opponent_disconnected')}
			</p>

			<h2 className={`${graceStyles.disconnectGraceTitle}`}>
				{secondsLeft}s
			</h2>

			<p className={`${graceStyles.disconnectGraceCopy}`}>
